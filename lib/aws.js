...
const toString = async () => {
  ...
  Object.keys(tasks).sort().forEach(account => {
    Object.keys(tasks[account]).sort().forEach(cluster => {
      ...
      text += `<div class="column"><br><br><u>${cluster} - ${tasks[account][cluster].length} instances - ${bastionHost.publicIp}</u>`;
      text += '<table>';
      tasks[account][cluster].sort((a,b) => a.name > b.name ? 1 : -1).forEach(task => {
        const uid = `${bastionHost.publicIp}${task.ip}`.replace(/\./g,'');
        const sshInfo = `ssh -t username@${bastionHost.publicIp} "ssh root@${task.ip}"`;
        const aOpen = `<a onclick="bold(this);showHide('${uid}');selectText('${uid}');">`;
        const aClose = `</a>`;
        text += `<tr><td class="fixed-width">${aOpen}${task.name}${aClose}</td><td>${aOpen}${task.ip}${aClose}</td><td style="display:none" id="${uid}">${sshInfo}</td></tr>`
      });
      text += '</table></div>';
    });
  });
